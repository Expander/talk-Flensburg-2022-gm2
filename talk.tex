...
\newcommand{\QC}[2]{%
  \begin{scope}[shift={#1},rotate around={#2:(0,0)},visible on=<2->]
...
\begin{frame}{}
  \begin{center}
    ...
    \href{https://en.wikipedia.org/wiki/File:Galaxy_rotation_under_the_influence_of_dark_matter.ogv}{\includegraphics[width=\textwidth]{videos/dark_matter_affecting_galaxy_rotation.png}}
...
\begin{frame}{\insertsection: origin}
  \begin{center}
    \begin{tikzpicture}
      ...
      \only<2>{%
        \draw[very thick,red,->] ($(n)+(0,-0.5)$) -- ++(0,1);
        \draw[very thick,green,->] ($(e)+(0,-1)$) -- ++(0,2);
      }%
      \only<1-2>{%
...
\note{
  ...
  \item To understand how an effective value of $g_e\neq 2$ is
    generated, we have to look at the mechanism that describes the
    interaction between the electron's magnetic moment and an external
    magnet.
  \end{itemize}
}

\begin{frame}[t]{Electron $g$-factor: Quantum Corrections}
  \begin{center}
    \begin{tikzpicture}[xcenter around = {0,0}{0,0}]
...
\begin{frame}{\insertsection: Fermilab (FNAL)}
  \begin{center}
    ...
    \href{https://youtu.be/rGLpMigWIIs}{\includegraphics[width=0.9\textwidth]{videos/Muon-g-2-moving.png}}
