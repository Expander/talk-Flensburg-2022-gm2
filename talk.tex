...
\note{
  ...
  \item To shed some light on Dark Matter, we'll start our journey
    ...
    ferromagnetism).
  \item We'll now look at a very interesting interesting pheomenon
...
\note{
  ...
  \item The value of the magnetic moment of the electron is given by
    ...
    $g$-factor.
  \item The classical quantum mechanics predicts $g_e=2$, which is a
    gigantic disagreement!
  \end{itemize}
}
...
\note{
  ...
  \item Then calculate how the electron interacts with a magnetic
    field.
  \item The magnetic field is mediated by a photon $\gamma$
...
\note{
  ...
  \item This quantum correction yields to a deviation from the value
    $g_e=2$.
  \item This relative \emph{deviation} is called the ``anomalous
    magnetic moment'' and denoted as $a_e$.
  \end{itemize}
}
