...
% a2 = 1165937e-9
% d2 = 12e-9
% combined = (a1/d1^2 + a2/d2^2) / (1/d1^2 + 1/d2^2)
% dcombined = sqrt(1 / (1/d1^2 + 1/d2^2))
...
% BNL-E821
\newcommand{\amuBNL}{11659209.1} % e-10
\newcommand{\numamuBNL}{\num{\amuBNL}}
...
\note{
  \begin{itemize}
  \item The experiment at FNAL consists of a proton $p^+$ beam, which
    hits a target, where they produce pions $\pi^+$.
  \item The pions decay into muons $\mu^+$.
  \item The result is a beam of polarized muons, which enter the
    ``Muon area''
  \item The idea is now: Move the muons around a circle until they
    decay, and infer $a_\mu$ from the decay products.
  \item We need a ring!
  \end{itemize}
}
...
\note{
  \begin{itemize}
  \item The transportation was extremely challenging.
...
\begin{frame}{\insertsection: Fermilab (FNAL)}
  \begin{center}
    \movie[width=0.9\textwidth,showcontrols]{\includegraphics[width=0.9\textwidth]{videos/Muon-g-2-moving.png}}{videos/Muon-g-2-moving.mp4}
...
\note{
  \begin{itemize}
  \item The polarized muons $\mu^+$ circle around the muon storage
    ring and begin to decay.
  \item Within the ring there is a magnetic field, which lets the
    muon's spins rotate.
  \item Depending on the spin direction, their decay products
    (neutrinos and positrons $e^+$) have different energy.
  \item Measuring the energy of the decay products allows the
    experimentalists to derive the value of $a_\mu$.
  \end{itemize}
}
...
\begin{frame}{\insertsection: Fermilab (FNAL)}
  [explain magnetic field in the ring and the decay of the muons into
  pions, show wiggly plot]
\end{frame}
...
\begin{frame}{\insertsection}
  ...
  \begin{align*}
    a_\mu^{\text{CERN}} &= (\numamuCERN \pm \DamuCERN)\times 10^{-10} && (1979)\\
...
\note{
  ...
  \item Problems: None of the new spin-partner particles have been
    observed yet. That is one reason the LHC was built.
  \end{itemize}
}
